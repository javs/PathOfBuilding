...
function ItemTradeClass:DoesItemMatchFilters(item)
...
function ItemTradeClass:SetSortMode(sortMode)
	self.sortMode = sortMode
	self:BuildSortOrder()
	self.listBuildFlag = true
end

function ItemTradeClass:BuildSortOrder()
...
function ItemTradeClass:Draw(viewPort)
...
function ItemTradeClass:GetRowValue(column, index, item)
	if column == 1 then
		return colorCodes[item.rarity] .. item.name
	end
end

function ItemTradeClass:AddValueTooltip(tooltip, index, item)
...
function ItemTradeClass:GetDragValue(index, item)
	return "Item", item
end

function ItemTradeClass:OnSelClick(index, item, doubleClick)
...
function ItemTradeClass:OnSelCopy(index, item)
	Copy(item.raw:gsub("\n","\r\n"))
end

function ItemTradeClass:OnHoverKeyUp(key)
