-- Path of Building
--
-- Class: Trade Item list
-- Build item list control.
--
local pairs = pairs
local t_insert = table.insert

local TradeItemListClass = newClass("TradeItemListControl", "ListControl", function(self, anchor, x, y, width, height, itemsTab)
...
function TradeItemListClass:GetRowValue(column, index, itemId)
...
function TradeItemListClass:AddValueTooltip(tooltip, index, itemId)
...
function TradeItemListClass:GetDragValue(index, itemId)
	return "Item", self.itemsTab.items[itemId]
end

function TradeItemListClass:ReceiveDrag(type, value, source)
	if type == "Item" then
...
function TradeItemListClass:OnOrderChange()
	self.itemsTab:AddUndoState()
end

function TradeItemListClass:OnSelClick(index, itemId, doubleClick)
	local item = self.itemsTab.items[itemId]
...
function TradeItemListClass:OnSelCopy(index, itemId)
	local item = self.itemsTab.items[itemId]
	Copy(item:BuildRaw():gsub("\n","\r\n"))
end

function TradeItemListClass:OnSelDelete(index, itemId)
	local item = self.itemsTab.items[itemId]
